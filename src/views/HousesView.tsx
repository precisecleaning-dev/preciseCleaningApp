...
export default function InvoicesView({ onOpenMenu, properties, setProperties, currentUser, activeRole, isSuperAdmin, onEditProperty }: InvoicesViewProps) {
  ...
  // ⭐ Edición en línea del pago de un empleado desde el detalle de la casa
  const [editingPayId, setEditingPayId] = useState<string | null>(null);
  const [payForm, setPayForm] = useState<Partial<PayrollRecord> | null>(null);
  ...
  // ⭐ PAGOS A EMPLEADOS: se rigen por el permiso del módulo Payroll, no por el
  //    de Houses. Con alcance 'Own' NO se muestran (vería el sueldo de otros).
  const payrollPerm = activeRole?.permissions?.find(p => p.module === 'Payroll');
  const canSeePayroll = isSuperAdmin
    || (!!payrollPerm?.canView && (payrollPerm?.scope as string) === 'All');
  const canEditPayroll = isSuperAdmin || (canSeePayroll && !!payrollPerm?.canEdit);
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // ⭐ PAGOS DE EMPLEADOS EN LA CASA (colección `payroll`)
  //    Se pueden ver y editar sin salir de Invoices. El total se recalcula
  //    igual que en PayrollView: base + extra - descuento.
  // ══════════════════════════════════════════════════════════════════════════
  const payrollsForHouse = (propertyId?: string) =>
    payrolls
      .filter(pay => pay.propertyId === propertyId)
      .sort((a, b) => {
        const ea = employees.find(e => e.id === a.employeeId);
        const eb = employees.find(e => e.id === b.employeeId);
        return `${ea?.firstName || ''} ${ea?.lastName || ''}`
          .localeCompare(`${eb?.firstName || ''} ${eb?.lastName || ''}`);
      });

  const employeeName = (employeeId?: string) => {
    const emp = employees.find(e => e.id === employeeId);
    if (!emp) return 'Empleado desconocido';
    return `${emp.firstName || ''} ${emp.lastName || ''}`.trim() || 'Sin nombre';
  };

  const payFormTotal = (f: Partial<PayrollRecord> | null) =>
    Number(f?.baseAmount || 0) + Number(f?.extraAmount || 0) - Number(f?.discountAmount || 0);

  const startEditPay = (pay: PayrollRecord) => {
    setEditingPayId(pay.id || null);
    setPayForm({
      baseAmount: Number(pay.baseAmount || 0),
      extraAmount: Number(pay.extraAmount || 0),
      extraNote: pay.extraNote || '',
  ...
  const saveEditPay = async () => {
    if (!editingPayId || !payForm) return;
    setSavingPay(true);
    try {
      const total = payFormTotal(payForm);
      await updateDoc(doc(db, 'payroll', editingPayId), {
        baseAmount: Number(payForm.baseAmount || 0),
        extraAmount: Number(payForm.extraAmount || 0),
        extraNote: payForm.extraNote || '',
        discountAmount: Number(payForm.discountAmount || 0),
        discountNote: payForm.discountNote || '',
        status: payForm.status || 'Pending',
        totalAmount: total,
      });
      // El listener onSnapshot de `payroll` refresca la lista y los totales solo.
      cancelEditPay();
    } catch (err) {
          ...
          <div className="modal-70" onClick={e => e.stopPropagation()}>
            ...
            <div className="inv-modal-body">
              ...
              <div className="grid-3-cols">
                ...
                {/* ⭐ PAGOS DE EMPLEADOS EN ESTA CASA — ver y editar sin salir de Invoices */}
                {canSeePayroll && (
                  <div className="col-span-full inv-payroll-box">
                    <div className="inv-payroll-header">
                      <span className="inv-detail-label">
                        <DollarSign size={14} className="inv-label-icon-inline" /> EMPLOYEE PAYMENTS
                      </span>
                      <span className="inv-payroll-total-chip">
                        Total: ${payrollsForHouse(detailHouse.id)
                          .reduce((s, pay) => s + getPayrollTotal(pay), 0)
                          .toFixed(2)}
                      </span>
                    </div>

                    {payrollsForHouse(detailHouse.id).length === 0 ? (
                      <div className="inv-payroll-empty">
                        Esta casa todavía no tiene pagos de nómina registrados.
                      </div>
                    ) : (
                      <ul className="inv-payroll-list">
                        {payrollsForHouse(detailHouse.id).map(pay => {
                          const isEditing = editingPayId === pay.id;
                          const isPaid = String(pay.status || '').toLowerCase() === 'paid';
                          return (
                            <li key={pay.id} className="inv-payroll-item">
                              <div className="inv-payroll-item-top">
                                <div className="inv-payroll-emp">
                                  <User size={13} color="#64748b" /> {employeeName(pay.employeeId)}
                                </div>
                                <div className="inv-payroll-right">
                                  <span className={`inv-payroll-status ${isPaid ? 'paid' : 'pending'}`}>
                                    {isPaid ? 'Paid' : 'Pending'}
                                  </span>
                                  <span className="inv-payroll-amount">
                                    ${getPayrollTotal(pay).toFixed(2)}
                                  </span>
                                  {canEditPayroll && !isEditing && (
                                    <button
                                      className="inv-payroll-edit-btn"
                                      onClick={() => startEditPay(pay)}
                              ...
                              {!isEditing ? (
                                <div className="inv-payroll-breakdown">
                                  <span>Base: ${Number(pay.baseAmount || 0).toFixed(2)}</span>
                                  {Number(pay.extraAmount || 0) > 0 && (
                                    <span className="extra">
                                      + Extra: ${Number(pay.extraAmount).toFixed(2)}
                                      {pay.extraNote ? ` (${pay.extraNote})` : ''}
                                    </span>
                                  )}
                                  {Number(pay.discountAmount || 0) > 0 && (
                                    <span className="discount">
                                      − Desc: ${Number(pay.discountAmount).toFixed(2)}
                                      {pay.discountNote ? ` (${pay.discountNote})` : ''}
                                    </span>
                                  )}
                                </div>
                              ) : (
                                <div className="inv-payroll-form">
                                  <div className="inv-payroll-form-grid">
                                    <div>
                                      <label className="inv-payroll-label">Base ($)</label>
                                      <input
                                        type="number" step="0.01" className="inv-payroll-input"
                                        value={payForm?.baseAmount ?? ''}
                                        onChange={e => setPayForm({ ...payForm, baseAmount: Number(e.target.value) })}
                                      />
                                    </div>
                                    <div>
                                      <label className="inv-payroll-label">Extra ($)</label>
                                      <input
                                        type="number" step="0.01" className="inv-payroll-input"
                                        value={payForm?.extraAmount ?? ''}
                                        onChange={e => setPayForm({ ...payForm, extraAmount: Number(e.target.value) })}
                                      />
                                    </div>
                                    <div>
                                      <label className="inv-payroll-label">Descuento ($)</label>
                                      <input
                                        type="number" step="0.01" className="inv-payroll-input"
                                        value={payForm?.discountAmount ?? ''}
                                        onChange={e => setPayForm({ ...payForm, discountAmount: Number(e.target.value) })}
                                      />
                                    </div>
                                    <div>
                                      <label className="inv-payroll-label">Estado</label>
                                      <select
                                        className="inv-payroll-input"
                                        value={String(payForm?.status || 'Pending')}
                                        onChange={e => setPayForm({ ...payForm, status: e.target.value as PayrollRecord['status'] })}
                                      >
                                        <option value="Pending">Pending</option>
                                        <option value="Paid">Paid</option>
                                      </select>
                                    </div>
                                    <div className="inv-payroll-form-wide">
                                      <label className="inv-payroll-label">Nota del extra</label>
                                      <input
                                        type="text" className="inv-payroll-input"
                                        placeholder="Motivo del extra..."
                                        value={payForm?.extraNote || ''}
                                        onChange={e => setPayForm({ ...payForm, extraNote: e.target.value })}
                                      />
                                    </div>
                                    <div className="inv-payroll-form-wide">
                                      <label className="inv-payroll-label">Nota del descuento</label>
                                      <input
                                        type="text" className="inv-payroll-input"
                                        placeholder="Motivo del descuento..."
                                        value={payForm?.discountNote || ''}
                                        onChange={e => setPayForm({ ...payForm, discountNote: e.target.value })}
                                      />
                                    </div>
                                  </div>

                                  <div className="inv-payroll-form-footer">
                                    <span className="inv-payroll-new-total">
                                      Nuevo total: <b>${payFormTotal(payForm).toFixed(2)}</b>
                                    </span>
                                    <div className="inv-payroll-form-actions">
                                      <button className="inv-payroll-cancel" onClick={cancelEditPay} disabled={savingPay}>
                                        Cancelar
                                      </button>
                                      <button className="inv-payroll-save" onClick={saveEditPay} disabled={savingPay}>
                                        {savingPay ? <Loader2 size={14} /> : <Save size={14} />} Guardar
                                      </button>
                                    </div>
                                  </div>
                                </div>
